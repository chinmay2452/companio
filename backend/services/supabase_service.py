...
def fetch_due_cards(user_id: str, limit: int = 20) -> list[dict]:
    try:
        ...
        ).eq("user_id", user_id).lte("next_review", today_str).order("next_review").order("ease_factor").limit(limit).execute()
        return response.data
    except Exception as e:
        print(f"Error in fetch_due_cards: {e}")
        raise

def update_card_after_review(card_id: str, ease_factor: float, interval_days: int, repetitions: int, next_review: str) -> dict:
    try:
...
def save_daily_plan(user_id: str, plan_date: str, plan: list) -> dict:
    try:
        data = {
            "user_id": user_id,
            "plan_date": plan_date,
...
def get_user_stats(user_id: str) -> dict:
    try:
        ...
        due_res = supabase.table("cards").select("id", count="exact").eq("user_id", user_id).lte("next_review", today_str).execute()
